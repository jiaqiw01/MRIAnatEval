...
from dataset import MRIDataset, train_val_split
        ...
        for i, t in enumerate(tqdm(reversed(time_steps), desc='sampling loop time step',
            ...
            if use_ddim:
                ...
                img = self.p_sample(img, batch_images_inputs_lr, time, indexes=indexes, cond=cond,
                                    cond_scale=cond_scale)
            interm_out.append(unnormalize_img(img).cpu().numpy())

        return unnormalize_img(img), interm_out
...
class Trainer(object):
    def __init__(
        ...
        all_dataset = MRIDataset(train=True, path="/scratch/project_2001654/Wpeng/data/MRI_Three/MRI_High/", shard=0, num_shards=0, T=num_frames, resize=False, crop=True, crop_size=crop_size)
        self.train_dataset, self.val_dataset = train_val_split(all_dataset, TRAIN_VAL_RATIO)
        ...
        self.dl_eval = cycle(
            data.DataLoader(self.val_dataset, batch_size=1, shuffle=True, pin_memory=True))
    ...
    def save(self, milestone):
        ...
        # torch.save(self.model.denoise_fn, str(self.results_folder / f'SR_unet_{milestone}.pt'))
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                with self.accelerator.accumulate(self.model):
                    img = next(self.dl)
                    # img_lr = F.interpolate(data, size=(144//3, 192//3, 144//3))

                    img = img.to(self.accelerator.device).squeeze(dim=2)

                    # img_lr = F.interpolate(img, size=(12, 16, 12), mode='nearest')
                    # img_lr = F.interpolate(img_lr, size=(72, 96, 72), mode='nearest')
                    img_lr = F.interpolate(img, scale_factor=0.25, mode='nearest')
                    img_lr = F.interpolate(img_lr, scale_factor=4, mode='nearest')
                    
                    # print(f"img lr: {img_lr.shape}")
                    # assert img_lr.shape == (1, 1, 72, 96, 72)

                    # text = text.to(self.accelerator.device)
                    text = None

                    B, C, D, H, W = img.shape
                    # print(img_lr.shape, img.shape)
                    assert img.shape == img_lr.shape

                    batch_images_inputs = []
                    batch_images_inputs_lr = []
                    indexes = []
                    for b in range(img.shape[0]):
                        index = np.arange(0, self.num_frames)
                        # index = np.arange(0, 48)
                    ...
                    batch_images_inputs = torch.cat(batch_images_inputs, dim=0).to(self.accelerator.device)
                    batch_images_inputs_lr = torch.cat(batch_images_inputs_lr, dim=0).to(self.accelerator.device)
                    noise = torch.randn_like(batch_images_inputs_lr)
                    alpha = np.random.rand(1).item()*0.4
                    batch_images_inputs_lr = ((1-alpha)*batch_images_inputs_lr + alpha*noise).to(self.accelerator.device)
            ...
            if self.accelerator.sync_gradients:
                ...
                with torch.no_grad():
                    if self.step != 0 and self.step % (self.save_and_sample_every) == 0:
                        ...
                        all_videos_list = list(map(lambda n: self.ema_model.sample(batch_size=n, img_lr=img_lr, cond=text), batches))
