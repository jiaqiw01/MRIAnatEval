...
class BaseDataset(Dataset):
    ...
    """
    def __init__(self, path, T, resize, crop, crop_size):
        super().__init__()
        self.T = T
        self.path = Path(path)
        self.is_test = False
...
class MRIDataset(BaseDataset):
    def __init__(self, path, train=None, shard=None, num_shards=None, T=None, resize=False, resized_shape=(144, 96, 72), crop=False, crop_size=(72, 96, 72)):
        super().__init__(path=path, T=T, resize=resize, crop=crop, crop_size=crop_size)
    ...
    def loaditem(self, path):
        ...
        if self.crop:
            ...
            d_idx = torch.randint(0, 144-d, (1,))
            h_idx = torch.randint(0, 192-h, (1,))
            w_idx = torch.randint(0, 144-w, (1,))
            img = img[d_idx:d_idx+d, h_idx:h_idx+h, w_idx:w_idx+w]
            # assert img.shape == (d, h, w)
            # data = F.interpolate(data[None], size=(144//3, 192//3, 144//3)).squeeze(0)
        else:
            assert img.shape == (144, 192, 144)
        # print(f'this is shape {data.shape}')
        data = th.from_numpy(img[None,:,:,:]).float()
        if self.resize:
            data = F.interpolate(data[None], size=self.resized_shape).squeeze(0)
        return data
