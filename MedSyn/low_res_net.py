...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def p_sample_loop(self, shape, cond=None, cond_scale=1., device='cuda'):
    ...
    @torch.inference_mode()
    def sample(self, cond=None, cond_scale=1., batch_size=2):
...
class Trainer(object):
    def __init__(
        ...
        self.train_dataset = MRIDataset(train=True, path="/scratch/project_2001654/Wpeng/data/MRI_Three/MRI_High/", shard=0, num_shards=0, T=num_frames, resize=True, resized_shape=(num_frames, image_size[0], image_size[1]))
        ...
        self.dl = cycle(data.DataLoader(self.train_dataset, batch_size=train_batch_size, num_workers=1, shuffle=True, pin_memory=True))
        self.opt = AdamW(diffusion_model.parameters(), lr=train_lr, betas=(0.9, 0.999), weight_decay=0.01)

        self.step = 25 * self.save_and_sample_every
    ...
    def save(self, milestone):
        ...
        torch.save(self.model.denoise_fn.state_dict(), self.results_folder / f'low_res_unet_{milestone}.pt')
    ...
    def train(
        ...
        summary_writer = SummaryWriter("./checkpoint/low_res")
            ...
            for i in range(self.gradient_accumulate_every):
                with self.accelerator.accumulate(self.model):
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        print(f'step {self.step}: avg_loss {avg_loss},  grad_norm {grad_norm}')
                ...
                if self.accelerator.sync_gradients:
                    ...
                    with torch.no_grad():
                        if self.step != 0 and self.step % (self.save_and_sample_every) == 0:
                            summary_writer.add_scalar("avg_loss", avg_loss, self.step)
                            summary_writer.add_scalar("grad_norm", grad_norm, self.step)
                            ...
                            all_videos_list = list(
                                map(lambda n: self.ema_model.sample(batch_size=n, cond=None), batches))
                            ...
                            featmask = np.squeeze((0.5*all_videos_list[0]+0.5).data.cpu().numpy())
                            featmask = nib.Nifti1Image(featmask.transpose((2,1,0)),affine = np.eye(4))
                            fig=plt.figure()
                            plotting.plot_img(featmask,title="FAKE",cut_coords=(48//2, 64//2, 48//16),figure=fig,draw_cross=False,cmap="gray")
                            summary_writer.add_figure('Fake', fig, self.step, close=True)
                            ...
                            print("Finished video tensor to gif")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("/scratch/project_2001654/Wpeng/JIAQI/2stage/results/low_144_96_72/low_res_unet_25.pt"))
    print("ckpt loaded....")

    diffusion_model = GaussianDiffusion(
        denoise_fn=model,
        image_size=(192//3, 144//3),
        num_frames=144,
        text_use_bert_cls=False,
        channels=1,
        timesteps=1000,
    ...
    trainer = Trainer(diffusion_model=diffusion_model,
                    ...
                    update_ema_every=2,
                    save_and_sample_every=1000,
                    results_folder='./results/low_144_96_72',
                    num_sample_rows=3,
                    max_grad_norm=1.0)
