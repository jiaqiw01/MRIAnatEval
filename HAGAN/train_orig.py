...
parser.add_argument('--num-iter', default=50000, type=int,
                    help='number of iteration for training (default: SET TO 5000)')
...
def main():
    ...
    print("Training 128-128-128 HA-GAN hyperparameters: ")
    ...
    # trainset = Volume_Dataset(data_dir=args.data_dir)
    trainset = MRIDataset(path=args.data_dir, resize=True)
    train_loader = torch.utils.data.DataLoader(trainset,batch_size=args.batch_size,drop_last=True,
                                               shuffle=False,num_workers=args.workers)
    gen_load = inf_train_gen(train_loader)
    
    from hagan_models.Model_HA_GAN_128 import Discriminator, Generator, Encoder, Sub_Encoder
    ...
    for iteration in range(args.continue_iter, args.num_iter):
        ...
        if iteration > 2000 and (iteration+1)%1000 == 0:
