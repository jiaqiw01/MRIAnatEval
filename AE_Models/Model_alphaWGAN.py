...
class Generator(nn.Module):
    def __init__(self, noise:int=100, channel:int=64):
        ...
        self.relu = nn.ReLU()
        self.noise = noise
        self.tp_conv1 = nn.ConvTranspose3d(noise, _c*8, kernel_size=(4,6,4), stride=1, padding=0, bias=False)
    ...
    def forward(self, noise):
        ...
        h = F.interpolate(h, scale_factor=(2.25, 2, 2.25))
        # h = self.tp_conv6(h)

        h = F.tanh(h)

        assert h.shape[2:] == (144, 192, 144)
