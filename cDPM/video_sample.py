...
def sample_video(args, model, diffusion, batch, just_get_indices=False):
    ...
    while True:
        ...
        # Inputs of diffuion model are image frames
...
def main(args, model, diffusion, dataset):
    ...
    while len(not_done) > 0:
        ...
        output_filenames = [args.eval_dir / "samples" / f"sample_{i:04d}-{args.sample_idx}.nii.gz" for i in batch_indices]
        ...
        print(f"Time of sampling: {dur}")
        exit()
        # np.save("generated_samples_no_norm.npy", samples)
        # samples = ((samples + 1) * 127.5).clamp(0, 255).to(th.uint8).cpu().numpy()
        print(f"Finished sampling.... shape {samples.shape}")
        # np.save("generated_samples_0_255.npy", samples)
        ni_img = nib.Nifti1Image(np.squeeze(samples[0].data.cpu().numpy()), affine=np.eye(4))
        nib.save(ni_img, output_filenames[0])
        ...
        #     if todo[i]:
        #         numpy_array = np.transpose(samples[i].numpy(), (1, 2,3, 0)).squeeze(0) # 144, 1, 192, 144
        ...
        #         # fig=plt.figure()
        #         # plotting.plot_img(numpy_array.squeeze(),title="FAKE",cut_coords=(144//2, 192//2, 144//2),figure=fig,draw_cross=False,cmap="gray")
        #         # plt.savefig(f"masked_fake_{i}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_obs", type=int, default=1, help="Number of observed frames at the beginning of the video. The rest are sampled.")
    ...
    print(f'There are {args.n_obs} observed frames')
    print(f"Sampling for indices {args.start_index} to {args.stop_index}.")

    # Load the checkpoint (state dictionary and config)
    data = dist_util.load_state_dict(args.checkpoint_path, map_location="cuda")
