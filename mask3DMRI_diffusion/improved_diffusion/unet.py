...
class UNetVideoModel(nn.Module):
    ...
    def forward(self, x, *, x0, timesteps, frame_indices=None,
        ...
        h = h.type(x.dtype)
        out = self.out(h)
        res = out.view(B, T, self.out_channels, H, W)
        # print("~~~~Returnning the results!~~~~")
        # print(res.shape)
        # assert res.shape[2:] == (144, 192, 144)
        return res, attns
...
#     inputs = torch.zeros((2, 1, 144, 192, 144))
#     m = UNetVideoModel(in_channels=1, model_channels=128, out_channels=1, num_res_blocks=2)
#     outs = m(inputs)
#     print(outs.shape)